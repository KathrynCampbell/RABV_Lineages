...
write.csv(sequence_data, file = (paste(args, "/", args, "_sequence_data.csv", sep = "")), row.names=F)
write.csv(node_data, file = (paste(args, "/", args, "_node_data.csv", sep = "")), row.names=F)
...
for (i in c(1:length(node_data$Node))) {
  plot_tree<-plot_tree +
    geom_cladelabel(node_data$Node[i], node_data$cluster[i], offset = 0.005*i, offset.text = 0)
}

ggsave(paste(args, "/", args, "_lineage_tree.png", sep = ""), 
       plot = plot_tree,
       height = 20, width = 30)
...
timeseries<- ggplot(sequence_data, aes(x=factor(Year), fill=cluster))+
  ...
  ggtitle(paste(args, "Time Series", sep = ""))

# Save it
ggsave(paste(args, "/", args, "_timeseries.png", sep = ""),
       plot = timeseries,
       height = 20, width = 30)
...
write.csv(clusters, file = (paste(args, "/", args, "_lineage_info.csv", sep = "")), row.names=F)
