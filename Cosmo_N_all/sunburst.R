...
node_data<-node_data[which(node_data$cluster %in% lineage_info$cluster),]
...
new<-plot_ly(
  ...
  width = 700,
  height = 700,
  marker = list(colors = list("#08306B", "#083776", "#083F82", "#08478E", "#08509A", "#0D57A1", 
                              "#135FA7", "#1967AD", "#1F6FB3", "#2776B8", "#2E7EBC", "#3686C0", 
                              "#3F8FC4", "#4896C8", "#529DCC", "#5CA3D0", "#66AAD4", "#71B1D7", 
                              "#7DB8DA", "#89BEDC", "#96C5DF", "#A1CBE2", "#ABCFE5", "#B5D3E9", 
                              "#DE2D26", "#E33D32", "#E84E3F", "#ED5F4C", "#F27058", "#F78165", 
                              "#FC9272", "#FC9F82", "#FCAC92", "#FDB9A2", "#FDC6B2", "#FDD3C2", 
                              "#FEE0D2", "#756BB1", "#7F77B7", "#8A84BE", "#9590C4", "#A09DCB", 
                              "#ABAAD2", "#B6B6D8", "#BFC0DD", "#C7C8E1", "#CFCFE5", "#D7D6E9", 
                              "#DFDEED", "#E7E5F1", "#EFEDF5", "#FFF7BC", "#1C9099", "#A6BDDB", 
                              "#ECE2F0", "#F03B20", "#FEB24C", "#FFEDA0", "#E34A33", "#EA6A4A", 
                              "#F18A61", "#F9AA78", "#FDC18D", "#FDCEA1", "#FDDBB4", "#FEE8C8", 
                              "#2B8CBE", "#A6BDDB", "#ECE7F2", "#FBB4AE", "#31A354", "#43AB5F", 
                              "#56B46B", "#69BE77", "#7BC783", "#8ED08F", "#A1D99B", "#ACDDA6", 
                              "#B7E2B2", "#C3E7BD", "#CEEBC9", "#D9F0D4", "#E5F5E0", "#636363", 
                              "#9F9F9F", "#CECECE", "#F0F0F0", "#43A2CA", "#75BFBF", "#A8DDB5", 
                              "#C3E8C8", "#E0F3DB", "#E5F5F9", "#C51B8A", "#E873A6", "#FBB4C2", 
                              "#FDE0DD", "#E6550D", "#E9621B", "#ED7029", "#F07E38", "#F48B46", 
                              "#F79955", "#FBA763", "#FDB272", "#FDBA81", "#FDC391", "#FDCCA0", 
                              "#FDD4AF", "#FDDDBE", "#FEE6CE", "#8856A7", "#E0ECF4", "#31A354", 
                              "#83C97A", "#C5E79C", "#F7FCB9", "#DD1C77", "#E7E1EF", "#2C7FB8", 
                              "#7FCDBB", "#EDF8B1", "#BED7EC", "#C7DBEF", "#CDDFF1", "#D3E3F3", 
                              "#D8E7F5", "#DEEBF7", "#E4EFF9", "#EAF3FB", "#F0F7FD", "#F7FBFF"
  ))
)
...
for (i in 1:length(lineage_table$country)) {
  lineage_table$radius[i]<-sum(lineage_table[i,2:133])
}
...
colour_table<-data.frame(lineage = colnames(lineage_table[2:133]), colour = NA)
...
plot_world<-plot + geom_scatterpie(aes(x=LON, y=LAT, group=region, r=radius),
                                   data=lineage_table, cols=c(colnames(lineage_table)[2:133]), color=NA, alpha=.8)+ 
  theme(legend.position = "none") + scale_fill_manual(values = c(colour_table$colour))+
  coord_sf(xlim = c(-155, 155), ylim = c(-50, 70))

plot_zoom<-plot + geom_scatterpie(aes(x=LON, y=LAT, group=region, r=(radius/2.5)),
                                  data=lineage_table, cols=c(colnames(lineage_table)[2:133]), color=NA, alpha=.8)+ 
  theme(legend.position = "none") + scale_fill_manual(values = c(colour_table$colour))+
