...
#'---------------------------------------------------------
#'title: Tanzanian WGS Lineage Assignment
...
outgroup<-node_data$Node[grep("SEA1b", node_data$cluster)]
tree<-root(tree, node = outgroup)
...
sequence_data<-
  sequence_data[-c(
    which(sequence_data$cluster == node_data$cluster[which(node_data$Node == outgroup)])),]
node_data<-node_data[-c(which(node_data$Node == outgroup)),]
#############################################
#                TIMESERIES                 #
#############################################
...
for (i in 1:length(sequence_data$ID)) {
  sequence_data$host[i] <- metadata$sequence.host[(which(metadata$ID == sequence_data$ID[i]))]
}

sequence_data$host<-gsub("Canis lupus familiaris", "Canis familiaris", sequence_data$host)
sequence_data$host<-gsub("unknown", "-", sequence_data$host)
sequence_data$host<-gsub("not available", "-", sequence_data$host)
...
for (i in 1:length(clusters$cluster)) {
  ...
  clusters$country[i]<-paste((sequence_data %>%
                                ...
                                group_by(Country) %>%
                                summarise()))
  clusters$hosts[i]<-getmode(sequence_data$host[which(sequence_data$cluster == clusters$cluster[i])])
}
